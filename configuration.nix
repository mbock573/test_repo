{ pkgs, ... }:

{
  nix.settings = {
    experimental-features = "nix-command flakes";
  };
  
  environment.systemPackages = [
    pkgs.vim
    pkgs.git
  ];

  # Add Nginx configuration
  services.nginx = {
    enable = true;
    
    # Basic default server (optional)
    virtualHosts."localhost" = {
      default = true;
      locations."/" = {
        root = pkgs.writeTextDir "index.html" "Hello from NixOS!";
      };
    };
  };

  # Add Docker configuration
  virtualisation.docker.enable = true;

  # Update firewall rules
  networking.firewall.allowedTCPPorts = [ 22 80 443 ];  # Added 80 (HTTP) and 443 (HTTPS)

  # Add docker group to your user
  users.users.mbock.extraGroups = [ "wheel" "docker" ];  # Added "docker" group

  # Original file system configuration
  fileSystems."/" = {
    device = "/dev/disk/by-label/nixos";
    fsType = "ext4";
  };
  fileSystems."/boot" = {
    device = "/dev/disk/by-label/boot";
    fsType = "ext4";
  };
  swapDevices = [
    {
      device = "/dev/disk/by-label/swap";
    }
  ];

  # Remainder of original configuration
  time.timeZone = "Europe/Berlin";
  i18n.defaultLocale = "de_DE.UTF-8";
  console.keyMap = "de";
  
  boot.loader.grub.enable = true;
  boot.loader.grub.device = "/dev/sda";
  boot.initrd.availableKernelModules = [ "ahci" "xhci_pci" "virtio_pci" "virtio_scsi" "sd_mod" "sr_mod" "ext4" ];
  
  users.users = {
    root.hashedPassword = "!";
    mbock = {
      isNormalUser = true;
      extraGroups = [ "wheel" "docker" ];  # Updated here as well for clarity
      openssh.authorizedKeys.keys = [
        "ssh-rsa  mbock@hp_elitebook"
      ];
    };
  };

  security.sudo.wheelNeedsPassword = false;
  
  services.openssh = {
    enable = true;
    settings = {
      PermitRootLogin = "no";
      PasswordAuthentication = false;
      KbdInteractiveAuthentication = false;
    };
  };

  system.stateVersion = "24.11";
}
